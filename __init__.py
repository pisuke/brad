...
bl_info = {
    "name": "Radiance 4.1",
    "author": "Francesco Anselmo, Lucio Boscolo",
    "version": (0, 0, 1),
    "blender": (2, 6, 2),
    "location": "Render > Engine > Radiance",
    "description": "Basic Radiance integration for blender",
    "warning": "this script is beta",
    "wiki_url": "https://github.com/pisuke/brad/wiki",
    "tracker_url": "https://github.com/pisuke/brad/issues",
    "category": "Render"}
...
###############################################################################
# Radiance Scene properties.
###############################################################################
class RenderRadianceSettingsScene(bpy.types.PropertyGroup):
    # File Options
    ...
    scene_name = StringProperty(
            name="Scene Name",
            description="Name of Radiance-Ray scene to create. Empty name will use the name of " \
                        "the blend file",
            default="", maxlen=1024)
    scene_path = StringProperty(
            name="Export scene path",
            # description="Path to directory where the exported scene (Radiance and INI) is created",  # Bug in Radiance-Ray RC3
    ...
    # Not a real Radiance option, just to know if we should write
    radio_enable = BoolProperty(
            name="Enable Radiosity",
            description="Enable Radiance-Rays radiosity calculation",
            default=False)
    ...
    media_enable = BoolProperty(
            name="Enable Media",
            description="Enable Radiance-Rays atmospheric media",
            default=False)
    ...
    baking_enable = BoolProperty(
            name="Enable Baking",
            description="Enable Radiance-Rays texture baking",
            default=False)
    ...
    comments_enable = BoolProperty(
            ...
            description="Add comments to Radiance file",
            default=True)

    # Real Radiance options
    ...
    antialias_method = EnumProperty(
            ...
            items=(("0", "non-recursive AA", "Type 1 Sampling in Radiance-Ray"),
                   ("1", "recursive AA", "Type 2 Sampling in Radiance-Ray")),
            default="1")
    ...
    antialias_gamma = FloatProperty(
            ...
            description="Radiance-Ray compares gamma-adjusted values for super sampling. Antialias " \
...
###############################################################################
# Material Radiance properties.
###############################################################################
class RenderRadianceSettingsMaterial(bpy.types.PropertyGroup):
    ...
    replacement_text = StringProperty(
            name="Declared name:",
            description="Type the declared name in custom Radiance code or an external " \
                        ".inc it points at. texture {} expected",
            default="")


###############################################################################
# Texture Radiance properties.
###############################################################################
class RenderRadianceSettingsTexture(bpy.types.PropertyGroup):
    ...
    tex_gamma_enable = BoolProperty(
            ...
            description="Notify some custom gamma for which texture has been precorrected " \
                        ...
                        "OS expected standard (see Radiance doc)",
            default=False)
    ...
    replacement_text = StringProperty(
            name="Declared name:",
            description="Type the declared name in custom Radiance code or an external .inc " \
                        "it points at. pigment {} expected",
            default="")


###############################################################################
# Object Radiance properties.
###############################################################################
class RenderRadianceSettingsObject(bpy.types.PropertyGroup):
    #Importance sampling
    ...
    replacement_text = StringProperty(
            name="Declared name:",
            description="Type the declared name in custom Radiance code or an external .inc " \
                        "it points at. Any Radiance shape expected e.g: isosurface {}",
            default="")


###############################################################################
# Camera Radiance properties.
###############################################################################
class RenderRadianceSettingsCamera(bpy.types.PropertyGroup):
    #DOF Toggle
    dof_enable = BoolProperty(
            name="Depth Of Field", description="EnableRadiance-Ray Depth Of Field ",
            default=False)
...
###############################################################################
# Text Radiance properties.
###############################################################################
class RenderRadianceSettingsText(bpy.types.PropertyGroup):
    custom_code = BoolProperty(
            name="Custom Code",
            description="Add this text at the top of the exported Radiance-Ray file",
            default=False)
...
def register():
    ...
    bpy.types.Scene.Radiance = PointerProperty(type=RenderRadianceSettingsScene)
    bpy.types.Material.Radiance = PointerProperty(type=RenderRadianceSettingsMaterial)
    bpy.types.Texture.Radiance = PointerProperty(type=RenderRadianceSettingsTexture)
    bpy.types.Object.Radiance = PointerProperty(type=RenderRadianceSettingsObject)
    bpy.types.Camera.Radiance = PointerProperty(type=RenderRadianceSettingsCamera)
    bpy.types.Text.Radiance = PointerProperty(type=RenderRadianceSettingsText)
...
def unregister():
    ...
    del bpy.types.Texture.Radiance
    del bpy.types.Object.Radiance
    del bpy.types.Camera.Radiance
    del bpy.types.Text.Radiance
